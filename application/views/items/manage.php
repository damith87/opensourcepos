...
<div class="span8 padding10">
    <div class="container">
        ...
        <h3 class="fg-white"><?php echo $this->lang->line('common_list_of') . ' ' . $this->lang->line('module_' . $controller_name); ?></h3>
        <a class="button large thickbox none" title="<?= $this->lang->line($controller_name . '_new') ?>" href="<?= "$controller_name/view/-1/width:$form_width"; ?>"><?= $this->lang->line($controller_name . '_new') ?> <i class="icon-apple place-left"></i></a>
        <a class="button large thickbox none" title="Import Items from Excel" href="<?= "$controller_name/excel_import/width:$form_width"; ?>"> Import Items from Excel <i class="icon-file-excel place-left"></i></a>

        <h4 class="fg-white">Search Item(s)</h4>

        <div id="titleTextImg" >
            <div class="text fg-white">Advanced Search Options :
                <a id="imageDivLink" href="javascript:show_hide_search_filter('search_filter_section', 'imageDivLink');" style="outline:none;">
                    <img src="<?php echo isset($search_section_state) ? ( ($search_section_state) ? base_url() . 'images/minus.png' : base_url() . 'images/plus.png') : base_url() . 'images/plus.png'; ?>" ></a>
            </div>
        ...
        <div id="search_filter_section" class="bg-white" style="display: <?php echo isset($search_section_state) ? ( ($search_section_state) ? 'block' : 'none') : 'none'; ?>;">
            <?php echo form_open("$controller_name/refresh", array('id' => 'items_filter_form', 'class' => 'form')); ?>
        ...
        <div class="pagination bg-white fg-black">
            <ul>
        <?php echo $this->pagination->create_links(); ?>
            </ul>
        </div>
        <div class="fg-white bg-dark opacity padding10 border">
            <h4 class="fg-white">Bulk Actions</h4>
            <?php echo anchor("$controller_name/delete", $this->lang->line("common_delete").' <i class="icon-cancel-2"></i> ', array('id' => 'delete','class'=>'button danger')); ?>
                <?php echo anchor("$controller_name/bulk_edit/width:$form_width", $this->lang->line("items_bulk_edit").' <i class="icon-pencil"></i>', array('id' => 'bulk_edit', 'title' => $this->lang->line('items_edit_multiple_items'),'class'=>'button info')); ?>
                <?php echo anchor("$controller_name/generate_barcodes", $this->lang->line("items_generate_barcodes").' <i class="icon-barcode"></i>', array('id' => 'generate_barcodes', 'target' => '_blank', 'title' => $this->lang->line('items_generate_barcodes'),'class'=>'button success')); ?>
        </div>
